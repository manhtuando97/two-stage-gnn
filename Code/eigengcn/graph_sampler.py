...
class GraphSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, G_list, graphs_list,num_pool_matrix, num_pool_final_matrix,features='default', normalize=True, assign_feat='default', max_num_nodes=0, norm = 'l2'):
        ...
        node_id = node_list[0]
        self.feat_dim = G_list[0].nodes[node_id]['feat'].shape[0]
        ...
        for G in G_list:
            ...
            if assign_feat == 'id':
                assign_feat = np.hstack((np.identity(self.max_num_nodes), self.feature_all[-1]))
                self.assign_feat_all.append(assign_feat)
                G.graph['assign_feats'] = assign_feat
            else:
                self.assign_feat_all.append(self.feature_all[-1])
                G.graph['assign_feats'] = self.feature_all[-1].copy()
            self.G_list.append(G)

        
        
        for idx in range(len(self.graphs_list)):
            ...
            self.G_list[idx].graph['adj']  = adj_padded
            self.G_list[idx].graph['feats'] = self.feature_all[idx].copy()
            self.G_list[idx].graph['label'] = self.label_all[idx]
            self.G_list[idx].graph['num_nodes'] = num_nodes
            self.G_list[idx].graph['assign_feats'] = self.assign_feat_all[idx].copy()
            ...
            for i in range(len(graph.graphs)-1):
                ...
                self.G_list[idx].graph[num_nodes_key] = num_nodes_
                ...
                self.G_list[idx].graph[adj_key] = adj_padded_
            ...
            for i in range(len(graph.layer2pooling_matrices)):
                if i == len(graph.layer2pooling_matrices)-1:
                    for j in range(self.num_pool_final_matrix):
                        ...
                        self.G_list[idx].graph[pool_adj_key] = pool_adj_padded
                else:
