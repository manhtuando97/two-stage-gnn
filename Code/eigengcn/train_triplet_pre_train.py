...
def train(dataset, model, args, same_feat=True, val_dataset=None, test_dataset=None, mask_nodes = True):
    ...
    # First training stage: based on triplet loss
    ...
    # Second training stage: fine-tuning
    # Initialization of the final classifier
    in_feat = args.output_dim
    pred_layers = []
    ...
    for epoch in range(args.num_epochs):
        ...
        while not tripletsampler_tr.end():
            ...
            ypred =model( a_h0, a_adj, adj_pooled_list, a_batch_num_nodes, batch_num_nodes_list, pool_matrices_dic)
            #label = Variable(torch.Tensor(a.graph['label'])).cuda()
            label = Variable(torch.LongTensor([int(a.graph['label'])])).cuda()
    ...
    result = evaluate(test_dataset_loader, model, args, name='Test', max_num_examples=None)
    test_acc = result['acc']
    result = evaluate(val_dataset_loader, model, args, name='Validation', max_num_examples=None)
    val_acc = result['acc']
