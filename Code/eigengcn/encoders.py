...
class GcnEncoderGraph(nn.Module):
    def __init__(self, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        # final classifier for the fine-tuning setting, initialized at identity map
        self.map_model = torch.nn.Identity()
    ...
    def construct_mask(self, max_nodes, batch_num_nodes): 
        ...
        packed_masks = [torch.ones(int(num)).cuda() for num in batch_num_nodes]
        batch_size = len(batch_num_nodes)
        out_tensor = torch.zeros(batch_size, max_nodes).cuda()
    ...
    def loss(self, pred, label, type='softmax'):
        ...
        if type == 'softmax':
            ...
            return torch.nn.MultiLabelMarginLoss()(pred, label_onehot).cuda()
    ...
    def forward(self, x, adj, adj_pooled_list, batch_num_nodes, batch_num_nodes_list, pool_matrices_dic ,  **kwargs):
        ...
        for i in range(len(self.pool_sizes)):
            ...
            if self.mask:
                ...
                embedding_mask = None
            adj_new = adj_pooled_list[i].type(torch.FloatTensor).cuda()
...
class Pool(nn.Module):
    ...
    def forward(self,x):
        ...
        for i in range(self.num_pool):
            ...
            pool_matrix = pool_matrix.type(torch.FloatTensor).cuda()
