...
class Graphs():
	...
	def coarsening_pooling(self, normalize = True):
		...
		for i in range(len(self.pooling_sizes)):
			result, adj_coarsened, pooling_matrices = self._coarserning_pooling_(adj, self.pooling_sizes[i],normalize)
			if (result == -1):
				return (-1)
...
########## Constructing pooling matrix
		...
		return 1, A_coarsened, pooling_matrices
