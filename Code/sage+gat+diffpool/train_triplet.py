...
def evaluate(train_dataset, val_dataset, model, args, iteration, name='Validation', max_num_examples=None, writer=False):
    model.eval()
    ...
    for c in train_dataset.keys():
        for graph in train_dataset[c]:
            ...
            out, feat = model(h0, adj, batch_num_nodes, assign_x=assign_input)
...
def evaluate_mlp(train_dataset, val_dataset, model, args, iteration, name='Validation', max_num_examples=None, writer=False):
    model.eval()
...
def train(dataset, model, args, iteration, same_feat=True, val_dataset=None, test_dataset=None, writer=None,
    ...
    if val_dataset is not None:
        val_result = evaluate_mlp(dataset, val_dataset, model, args, iteration, name='Validation')
        val_acc = val_result['acc']
    if test_dataset is not None:
        test_result = evaluate_mlp(dataset, test_dataset, model, args, iteration, name='Test')
    ...
    for i in range(args.num_iterations):
        ...
        if args.method == 'soft-assign':
            ...
            model = encoders_GAT.DGATEncoderGraph(
                    input_dim, args.hidden_dim, args.output_dim, args.num_classes, 
                     num_layers = args.num_gc_layers, args=args, final_dim = 'output_dim').cuda()
        ...
        if args.val == True:
            _, test_acc, val_acc = train(train_dataset, model, args, i, val_dataset=val_dataset, test_dataset=test_dataset, writer=writer)
            all_vals.append(val_acc)
            all_tests.append(test_acc)
        else:
            _, test_acc, val_acc = train(train_dataset, model, args, i, val_dataset=None, test_dataset=test_dataset, writer=writer)
            all_tests.append(test_acc)
...
def taxi_task_val(args, writer=None, feat='learnable'):
    ...
    for i in range(args.num_iterations):
        ...
        if args.val == True:
            _, test_acc, val_acc = train(train_dataset, model, args, i, val_dataset=val_dataset, test_dataset=test_dataset,
            writer=writer)
            all_vals.append(val_acc)
            all_tests.append(test_acc)
        else:
            _, test_acc, val_acc = train(train_dataset, model, args, i, val_dataset=None, test_dataset=test_dataset,
            writer=writer)
            all_tests.append(test_acc)
