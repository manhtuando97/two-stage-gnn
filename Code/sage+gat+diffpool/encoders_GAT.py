...
class DGATLayer(nn.Module):
    def __init__(self, input_dim, output_dim, dropout=0.0, neg_input_slope=0.2, n_heads=4, concat=True):
        ...
        self.attentions = [DGATHead(input_dim, output_dim, dropout=dropout, neg_input_slope=neg_input_slope, concat=self.concat) for _ in range(n_heads)]
...
class DGATEncoderGraph(nn.Module):
    def __init__(self, input_dim, hidden_dim, embedding_dim, label_dim, args,
                       num_layers=2, num_heads=[2, 2], pred_hidden_dims=[], neg_input_slopes=[0.2, 0.2], dropouts=[0.0, 0.0], final_dim='output_dim', concat=True):
        ...
        # final classifier, used for the fine-tune setting, initialized to be the identity map:
        self.map2_model = torch.nn.Identity()
    ...
    def build_conv_layers(self, input_dim, hidden_dim, embedding_dim, num_layers,
        ...
        if num_layers >= 3:
            ...
            conv_block = None
        conv_last = DGATLayer(input_dim=hidden_dim*num_heads[-1], output_dim=embedding_dim, n_heads=num_heads[-1], dropout=dropouts[-1], concat=False)
    ...
    def forward(self, x, adj, batch_num_nodes=None, **kwargs):
        ...
        if self.final_dim != 'output_dim':
            output = self.pred_model(x)
            output = self.map2_model(output)
            return x, output
        else:
            output = self.map_model(x)
            output = self.map2_model(output)
            return x, output
