...
for iter in range(args.iterations):
    ...
    for epoch in range(args.epochs):
        ...
        while not tripletsampler_tr.end():
            ...
            dist_p, dist_n, embed_a, embed_p, embed_n = TNet(one_triplet['anchor'].to(device), one_triplet['pos'].to(device), one_triplet['neg'].to(device))
    ...
    # validation result
    val_result = evaluate_mlp(training_set, validation_set, model, device)
    val_accs.append(val_result['acc'])

    # test result
    test_result = evaluate_mlp(training_set, test_set, model, device)
